...
bool isNullFeature(vector<string> in){
	int matched = 0;
...
void trainData(){
	cout<<wordCount<<" this is the FVS"<<endl;
	numInputs = wordCount;
	numOutputs = 3;
	arrangement = vector<int>{numInputs,5,4, numOutputs};
	...
	for (int i = 0 ; i < trainingData.size(); i++){
		if(!isNullFeature(trainingData[i])){
			input.push_back(createInputVector(trainingData[i]));	
			if(trainingOutput[i][1] == 1){
				obj++;
			}
			if(trainingOutput[i][0] == 1){
				neg++;
			}
			if(trainingOutput[i][2] == 1){
				pos++;
			}
		}
		else{
			kicked++;
			trainingOutput.erase(trainingOutput.begin()+i);
			cout<<"kicked out"<<endl;
		}
		
	}

	cout<<"total kicked out "<<kicked<<endl;
	// cout<<"obj are "<<obj<<endl;
	// cout<<"pos are "<<pos<<endl;
	// cout<<"neg are "<<obj<<endl;
	// exit(0);
...
void testOnData(){
	...
	for(int i = 0 ; i<testInput.size(); i++){
		...
		if(maximum == myNeuronGrid.Grid.back()[0].getOutput()){
			numNegativeGiven++;
			if(i<5)printit.push_back("NEGATIVE");
		}
		else if(maximum == myNeuronGrid.Grid.back()[1].getOutput()){
			numObjectiveGiven++;
			if(i<5)printit.push_back("OBJECTIVE");
		}
		else{
			numPositiveGiven++;
			if(i<5)printit.push_back("POSITIVE");
		}
	...
	// cout<<"CORRECTIVE OBJECTIVE "<<(double)correctObjective<<endl;
